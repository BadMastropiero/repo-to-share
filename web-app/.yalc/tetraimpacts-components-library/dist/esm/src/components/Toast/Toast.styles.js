...
const StyledToast = styled.div(({ theme: { colors: { primaryBg, primaryLight }, }, background = primaryLight, }) => css `
    display: flex;
    flex-shrink: 0;
    flex-grow: 0;
    flex-direction: row;
...
`;
export const ToastHeader = styled.div(({ isSnackbar = false }) => css `
    display: flex;
    flex-shrink: 0;
    flex-direction: row;
    column-gap: 16px;
...
export const ToastButtonsContainer = styled.div `
  ...
  align-items: center;
`;
export const ToastButton = styled(Button)(({ highlightColor = 'white', hoverColor }) => css `
    border: unset;
  ...
  `);
export const ToastIcon = styled(Icon)(({ correctAlign = false, isSnackbar = false }) => css `
    flex-shrink: 0;
    align-self: center;
