...
export function getStyles({
  ...
  const style: StylesConfig<TiOptions> = {
    control: (css, state) => ({
      ...css,
      ...
      borderBottom: `${state.selectProps.menuIsOpen ? '0px' : ''}`,
      ':hover': {
        boxShadow: 'none',
        color: 'none',
        background: `${state.selectProps.primary ? colors.primary : colors.primaryBg}`,
    ...
    input: (css, state) => ({
      ...css,
      ...TiDdFontStyles,
      color: `${state.selectProps.primary ? colors.primaryLight : colors.primary200}`,
    }),

    loadingIndicator: (css, state) => ({
      ...css,
      color: `${state.selectProps.primary ? colors.primaryBg : colors.primaryLight}`,
    }),
    ...
    dropdownIndicator: (css, state) => ({
      ...
      minHeight: '36px',
      height: '36px',
      zIndex: `${state.selectProps.menuIsOpen ? '2' : 'unset'}`,
    }),
    ...
    multiValue: (css, state) => ({
      ...
      gap: '5px',
      borderRadius: 5,
      border: `2px solid ${state.selectProps.primary ? colors.primary25 : colors.primary600}`,
      backgroundColor: `${state.selectProps.primary ? colors.primaryBg : colors.primary}`,
    ...
    multiValueLabel: (css, state) => ({
      ...
      ...TiDdFontStyles,
      color: `${
        (state.selectProps.isDisabled && colors.primaryLight) ||
    ...
    indicatorSeparator: css => ({
      ...css,
      class: 'ti-indicatorSeparator', // TODO test it must not appear
      alignSelf: 'stretch',
    ...
    menu: (css, state) => ({
      ...
      boxShadow: 'none',
      backgroundColor: 'none',
      ':hover': {boxShadow: 'none', color: 'none', backgroundColor: 'none'},
      margin: 0,
      padding: 0,
      borderRadius: `${state.selectProps.menuIsOpen ? '0 0 10px 10px' : '10px'}`,
      border: `4px solid ${state.selectProps.primary ? colors.primary600 : colors.primary100}`,
      borderTop: `${state.selectProps.menuIsOpen ? '0px' : ''}`,
    }),
